...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="//ds:subsystem/ds:datasources/ds:datasource[@jndi-name='java:jboss/datasources/KeycloakDS']">
        <ds:datasource jndi-name="java:jboss/datasources/KeycloakDS" enabled="true" use-java-context="true" pool-name="KeycloakDS" use-ccm="true">
            <ds:connection-url>jdbc:postgresql://${env.KEYCLOAK_POSTGRES_HOST}:${env.KEYCLOAK_POSTGRES_PORT:5432}/${env.KEYCLOAK_POSTGRES_DATABASE}</ds:connection-url>
            <ds:driver>postgresql</ds:driver>
            <ds:security>
              <ds:user-name>${env.KEYCLOAK_POSTGRES_USER}</ds:user-name>
              <ds:password>${env.KEYCLOAK_POSTGRES_PASSWORD}</ds:password>
            </ds:security>
            <ds:validation>
